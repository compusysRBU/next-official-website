...
export default function ImageTrail({ items = [], variant = 1 }: ImageTrailProps) {
  ...
  return (
    <div className="w-full h-full relative z-[100] rounded-lg bg-transparent overflow-visible" ref={containerRef}>
      {items.map((url, i) => (
        <div
          className="content__img w-[150px] aspect-[1.1] rounded-[15px] absolute top-0 left-0 opacity-0 overflow-hidden [will-change:transform,filter]"
          key={i}
        >
          <div
            className="content__img-inner bg-center bg-cover w-[calc(80%+20px)] h-[calc(80%+20px)] absolute top-[-10px] left-[-10px]"
